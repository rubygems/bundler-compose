...
module Spec
  module Matchers
    ...
    RSpec::Matchers.define :match_fs do |expected|
      match(notify_expectation_failures: true) do |actual|
        e = expected.dup
        aggregate_failures do
          unexpected = []
          actual.bundle_dir.find do |path|
          ...
          expect(unexpected).to be_empty, "found unexpected files #{unexpected} in #{actual.bundle_dir}"
          expect(e.keys).to be_empty, "missing expected #{e.keys} files in #{actual.bundle_dir}"
        end
      end

      # failure_message do |actual|
      #   actual.bundle_dir.find.map do |path|
      #     next if path.directory?
